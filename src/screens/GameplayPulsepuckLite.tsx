...
export function GameplayPulsepuckLite({ actions, runtime }: GameplayPulsepuckLiteProps) {
  ...
  return (
    <>
      ...
      <p className="font-hud-display text-hud-display text-primary neon-text-primary">024,590</p>
      </div>
      <div className="hud-panel px-panel-padding py-2 rounded">
      <p className="font-hud-label text-hud-label text-outline uppercase">LEVEL</p>
      <p className="font-hud-display text-hud-display text-secondary neon-text-secondary">03</p>
      </div>
      </div>
      ...
      <div className="w-32 h-4 bg-tertiary neon-border rounded-full mx-auto mb-8 shadow-[0_0_15px_rgba(216,218,255,0.6)] cursor-pointer"></div>
      </div>
      {/* Start/Restart Controls Overlay (Optional state demonstration) */}
      <div className="absolute bottom-margin-compact left-1/2 -translate-x-1/2 z-20 flex gap-gutter">
      ...
      <div className="absolute inset-0 bg-background/80 backdrop-blur-xl z-50 flex items-center justify-center hidden" id="pauseOverlay">
